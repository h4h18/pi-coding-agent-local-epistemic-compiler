...
test("two hourly jobs on the same root append and unwrap DEK with recovery and online-wrap keys", async () => {
  ...
  try {
    ...
    const masterRaw = JSON.parse(readFileSync(path.join(secretDir, "restic-master.json"), "utf8")) as {
      encrypt: string;
      macK: string;
      macR: string;
    };
    ...
    expect(unitJson).not.toContain(readFileSync(path.join(secretDir, "online-wrap.pkcs8")).toString("base64"));
