...
test("selected manifests are listed in pins even when the selected set is empty", async () => {
  const pins = await loadPinMap(path.join(repoRoot(), "faex1", "config", "models", "pins.json"));
  expect(pins["faex1/config/models/selected.json"]).toMatch(/^sha256:[0-9a-f]{64}$/);
...
test("pin mismatch fails when a selected or production-ready file byte changes", async () => {
  ...
  await mkdir(path.join(dir, "faex1", "config", "models"), { recursive: true });
  const selectedPath = path.join(dir, "faex1", "config", "models", "selected.json");
  await writeFile(selectedPath, '{"kind":"selected-set","schemaVersion":1,"selectedIds":[]}\n');
  const digest = await sha256File(selectedPath);
  await writeFile(
    path.join(dir, "faex1", "config", "models", "pins.json"),
    `${JSON.stringify({ "faex1/config/models/selected.json": digest }, null, 2)}\n`,
  );
...
test("committed pins match current faex1/config/models bytes for every pinned path", async () => {
  const root = repoRoot();
  const pins = await loadPinMap(path.join(root, "faex1", "config", "models", "pins.json"));
